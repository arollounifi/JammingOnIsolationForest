...
class TwoRandom_JammingAttack(JammingAttack):
    ...
    def insert_attack(self, attack_type, index):
        ...
        new_burst_duration = self.decideBurstDuration() * 100
        new_duty_rate = self.decideDutyRate(attack_type)
        new_rest_duration = round((new_burst_duration / new_duty_rate) * (1 - new_duty_rate))

        self.restDurations[attack_type] = new_rest_duration
        self.burstDurations[attack_type] = new_burst_duration
        self.dutyRates[attack_type] = new_duty_rate
